...
class _StudentFormScreenState extends State<StudentFormScreen> {
  ...
  Future<void> _submit() async {
    if (_state == _SubmitState.submitting) return;
    ...
    try {
      ...
      if (_isDesktop) {
        // GPS not available on Linux/Windows/macOS desktop.
        // If the session has geofencing enabled, we can't satisfy it — tell the user.
        if (_session?.geoLat != null) {
          throw Exception(
            'GPS is not available on this device. '
            'This session requires location verification — '
            'please use a mobile device or web browser.',
          );
        }
        // No geofencing: server ignores lat/lon, submit with zeros.
      } else {
    ...
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Text(
          ...
          style:
              TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.25)),
        ),
      ]),
    );
  }
